...
;
; This version works with test probes plugged into the OHM jumper (ADC3).
;
; How to calculate resistance values:
;
; Read the two ADC values -- the high byte and low byte.  
; The following python function does the calculation, where "high" is the high
; byte, "low" is the low byte, and "R" is the known resistor value in the
; divider, and "VCC" is VCC.  The value 2.56 comes from the band gap reference
; voltage, this is configured in the ADMUX register.
;
; def ohms(high, low, R, VCC):
;     vout = (high * 256 + low) / 1024. * 2.56
;     return R * vout / (VCC - vout)
;
; If using a 10K resistor and 3.3V regulator, this is good for measured
; resistances up to 34.444K ohms.  Higher resistances (or infinite resistance
; as when there is no connection) will just show up as 34.444K.
